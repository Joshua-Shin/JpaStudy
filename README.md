...
#### 엔티티 매핑
- Order(주문), OrderItem(주문상품), Item(상품)의 관계
  - 주문이 있고 상품이 있어.
  - 하나의 주문 안에는 여러 상품들이 있을 수 있고,
  - 하나의 상품은 여러 주문에 속할 수 있어.
  - 즉 주문과 상품은 다대다 관계이기 떄문에 이를 풀어내기 위해 주문상품이라는 연관테이블을 만든거야.
  - 그렇게 해석해야지 주문상품 테이블이 실생활의 엔티티와 어떻게 매핑되는것인가 해석할 필요 없어. 마땅치 않아.
